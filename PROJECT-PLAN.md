...
| T-002 | 🔲 Open | High | Build the global style foundation (the workbench) — extract brand tokens, type scale, color, spacing, and default-content styling from the live semrush.com homepage at Refined fidelity. Record tokens in PROJECT-DESIGN.md. | styles/styles.css, PROJECT-DESIGN.md | `:root` tokens populated; type scale + color + spacing recorded; detect.mjs clean |
...
| T-004 | 🔲 Open | High | Style the homepage block-by-block to match the original (Refined), pass GATE 2. | blocks/*, styles/* | Visual critique similarity high; user validates |
