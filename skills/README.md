...
| [debug-eds-publish](debug-eds-publish/SKILL.md) | "preview org/site is not configured"; publish 404 / content-bus error; nav/footer 404 or blank only after publish; "works locally but not on aem.live" — routes each symptom to its real cause (client settings vs fstab/Code-Sync vs `/content/` path vs stale cache) |
...
| [styleguide-generator](styleguide-generator/SKILL.md) | Foundation is set and you want a Storybook-like living reference + GATE-2 diff target (all default content, every block × variant × section style, page templates); after validating a new block/variant/section-style/template; a no-render stage needs something reviewable. **Also owns the author-facing block library (DA) / component definition (UE) — its twin, created + maintained in lockstep on build-fresh projects** |
| [author-library-setup](author-library-setup/SKILL.md) | Wiring up or updating the author-facing block library — DA (da.live) Library panel config (`/.da/config` `library`+`blocks` sheets) or UE/CrossWalk component definition (`component-definition`/`models`/`filters.json`); deciding what belongs in the author palette vs the developer styleguide |
...
| [verify-publish-readiness](verify-publish-readiness/SKILL.md) | Project init (called by bootstrap-project) or before any "publish via Console" handoff — verify fstab mountpoint, preview org/site, and no `/content/` paths; hand off the exact user steps if a precondition is missing (EMA verifies, the user connects) |
| [migration-orientation](migration-orientation/SKILL.md) | **First thing on any new project** — user says "let's migrate X / start a new site / start a migration"; `PROJECT-DESIGN.md` has no `## Migration Strategy` section. Runs the setup conversation (authoring model, scope, analysis approach, starting page, sources, fidelity, resources, **existing-blocks + reuse/mutation bar**, overrides, constraints) and records the strategy before anything else happens |
...
| [post-import-sectionizer](post-import-sectionizer/SKILL.md) | An imported page renders as ONE section; `<hr>` breaks vanish from `.plain.html`; Section Metadata won't apply its style — restore boundaries with the deterministic post-import sectionizer (html2md strips `<hr>` wherever emitted) |
...
**Trigger `excat-visual-critique` aggressively for *discovery* — but it is the STRUCTURAL signal, not the gate.** The v2 critique is **extraction-based**: it detects exact CSS/structural/content/interaction diffs and computes a weighted similarity %, in **Block / Section / Page / Site** modes (site mode runs **parallel sub-agents, one per template**). Run it at the START of any styling task and again to confirm a fix moved the %. It **reports, never writes CSS**: critique discovers + scores the gaps, then the project's `block-visual-iteration` measure-first loop closes each one. **But it extracts from the static `.plain.html` — it is blind to rendered-pixel defects (overflow, clipping, overlap, misalignment), so a broken page can pass at ≥90%.** At GATE 2 it is therefore **one of three signals**, paired with a **rendered-screenshot + vision critique** (the layered diff in `validation-gates`); never pass a page on the % alone. Wired into `block-visual-iteration` (Step 0 + the Step 5 render check), `eds-migration-process` (GATE 2), and `styling-additively`.
...
| `excat-visual-critique` | **Aggressively, at the START of (and after) ANY styling pass** — extraction-based similarity % + categorized diffs vs original; Block/Section/Page/Site modes (site = parallel per-template sub-agents) | **Structural-discovery signal** — discovers + scores gaps (never writes CSS); **blind to rendered-pixel defects**, so at GATE 2 it's one of three, paired with the rendered-screenshot + vision critique (`validation-gates`); `block-visual-iteration` / `styling-additively` close the gaps |
