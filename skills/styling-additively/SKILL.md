...
**Step 1 — Toolbox-First: reproduce the look with what already exists.** Before writing any CSS, take stock of every block, variant, section style, and combination you already have — answer "what tools exist?" **mechanically**, not from memory: `ls blocks/` for the block census, plus `PROJECT-BLOCKS.md` for variants/section-styles. Try to reproduce the original page's look by *only* choosing among them: rename a block, switch to an existing variant, add a section style, combine them. Re-import / re-author the content with those choices. Even a content-validated page deserves this step — its structure may match the original with just different block names or variants. **Threshold to forge a new tool:** you can state, in one sentence, the specific look the existing toolbox cannot express. If you can't name what's missing, Step 1 isn't finished.
...
**The Additive-or-Ask Rule.** Touching shared or frozen-page code is allowed **iff** it's *provably additive* (the new rule targets a shape validated pages don't contain) **AND** *regression-verified* (measured before/after, or the shared block's critique % held). If you can't prove **both**, it's a frozen-page change — **stop and ask first** per `unfreeze-page` (the user reopens with `✅ 🔓`); never edit a frozen page's shared style on your own judgment. Working by "it's probably fine" is how a validated page silently moves.
