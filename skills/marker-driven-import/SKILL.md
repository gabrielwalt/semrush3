...
## Driver pattern — rebuild from wrappers (default)
Two ways the driver can assemble the output:
- **Parse-in-place** (`element.replaceWith()`): transforms matched regions but **leaves unmatched source content in place** → leaks (a duplicate `<h1>`, a stray flat list, nav/footer cruft) into the output. The A/B's Compare-Plans duplication + leftover headings came from this.
- **Rebuild-from-wrappers (default):** each parser **returns its wrapper**; the driver assembles a fresh `main` from **only the returned wrappers, in order**, discarding everything no parser claimed. Nothing unparsed can leak — clean by construction.

Default to **rebuild-from-wrappers** for any page with chrome or mixed content; reserve parse-in-place only for a page that is entirely block regions with no stray content. The validation loop below catches leaks either way, but rebuild-from-wrappers prevents them up front.
...
1. Import the validated page's URL to a **temporary location** (e.g. `/tmp/import-check/`), never `content/`. `run-bulk-import.js` writes to `content/` directly and has no `--output-dir`, so always back up first (see Pitfalls for the `cp`/restore commands).
...
- **Author-edit loss (remote, not just local).** A re-import/republish overwrites edits an author made **upstream** — DA admin PUT has **no merge semantics** and silently discards them. The local `cp` backup protects *your* curated reference; it does nothing for the author's work. Before re-importing a page that may carry author edits, the **user** should GET + diff the remote first (the agent is no-publish — this is human-executed guidance).
...
See also: `importer-parser-patterns` (table/DOM mechanics), `eds-content-modeling` (what the markers should map TO — the augmented-styles ladder), `eds-migration-process` (where this fits in the per-page workflow), `post-import-sectionizer` (restore the section boundaries html2md flattens — run after every import).
